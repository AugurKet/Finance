...
document.getElementById('calcBtn').addEventListener('click', () => {
    ...
    for (let year = 1; year <= (99 - entryAge + 1); year++) {
        ...
        // --- NEW PREMIUM ALLOCATION LOGIC BY TERM ---
        let allocationRate = 1.0;
        
        if (payingTerm === 6) {
            if (year === 1) allocationRate = 0.70;
            else if (year === 2) allocationRate = 0.80;
            else if (year === 3) allocationRate = 0.90;
            else if (year >= 4 && year <= 6) allocationRate = 1.02;
        } 
        else if (payingTerm === 10) {
            if (year === 1) allocationRate = 0.65;
            else if (year === 2) allocationRate = 0.67;
            else if (year === 3) allocationRate = 0.80;
            else if (year === 4) allocationRate = 0.90;
            else if (year === 5) allocationRate = 0.98;
            else if (year >= 6) allocationRate = 1.00;
        } 
        else if (payingTerm === 20) {
            if (year >= 1 && year <= 3) allocationRate = 0.60;
            else if (year >= 4 && year <= 6) allocationRate = 0.80;
            else if (year >= 7 && year <= 😎 allocationRate = 0.95; 
            else allocationRate = 1.00;
        }
        // --------------------------------------------
